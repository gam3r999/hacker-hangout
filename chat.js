...
function renderMessages(arr){
  ...
  arr.forEach(m=>{
    ...
    if(m.file) div.innerHTML=`<strong>${escapeHtml(m.username)}</strong>: <a href="${m.file}" target="_blank">[File]</a>`;
  ...
  if(file){
    ...
    storageRef.put(file).then(snapshot=>{
      snapshot.ref.getDownloadURL().then(url=>{
        messagesRef.push({username, text:text||"[File]", file:url, timestamp:Date.now(), deviceId});
        msgEl.value='';
        fileEl.value='';
      });
    });
