...
function loginAdmin() {
  ...
  if(admins[u] && admins[u]===p){ adminPanel.style.display='block'; sessionStorage.setItem('hh_admin_user',u); alert('Logged in as '+u); }
  else alert('Wrong credentials');
  adminUserEl.value=''; adminPassEl.value='';
}
function banUser(){ const u=(banUserEl.value||'').trim(); if(!u)return; banned[u]=true; db.ref('banned/'+u).set(true); updateBannedListUI(); banUserEl.value=''; }
function unbanUser(){ const u=(unbanUserEl.value||'').trim(); if(!u)return; delete banned[u]; db.ref('banned/'+u).remove(); updateBannedListUI(); unbanUserEl.value=''; }
function clearChat(){ if(!confirm('Clear all messages?')) return; db.ref('messages').remove(); messages=[]; renderMessages(); }
function nuke(){
  const bomb=document.createElement('div'); bomb.style.width='50px'; bomb.style.height='50px'; bomb.style.position='fixed'; bomb.style.top='0'; bomb.style.left='0';
  bomb.style.backgroundImage="url('https://i.imgur.com/nHg0JSz.png')"; bomb.style.backgroundSize='contain'; bomb.style.backgroundRepeat='no-repeat'; bomb.style.zIndex=9999;
  document.body.appendChild(bomb);
  new Audio('https://www.soundjay.com/misc/sounds/bomb-explosion-01.mp3').play();
  let x=0,y=0;
  const interval=setInterval(()=>{ x+=10; y+=10; bomb.style.transform=`translate(${x}px,${y}px)`; if(x>window.innerWidth/2){ clearInterval(interval); document.body.removeChild(bomb);
    ...
    setTimeout(()=>document.body.style.transform='translate(0,0)',300000);}},30); // 5 min cooldown
}
...
// ===== Sahur Cursor Mode (tung tung tung) =====
function startSahurMode(){
  sahurCursor=setInterval(()=>{
    document.body.style.cursor='crosshair'; // example effect
  },100);
}
function stopSahurMode(){ clearInterval(sahurCursor); document.body.style.cursor="url('https://i.imgur.com/nHg0JSz.png'), auto"; }
